...
function evaluation_window(fact::Vector{Int}, detection::Vector{Int}; window = 0)
    EvaluationResult(rtt_benchmark.evaluation_window(
        PyVector(fact),
        PyVector(detection),
        window,
    ))
...
function evaluation_window_weighted(
    trace::Vector{Float64},
    fact::Vector{Int},
    detection::Vector{Int};
    window = 0,
)
    WeightedEvaluationResult(rtt_benchmark.evaluation_window_weighted(
        trace,
        PyVector(fact),
        PyVector(detection),
        window,
...
f1(e::Union{EvaluationResult,WeightedEvaluationResult}) =
    2 * (e.precision * e.recall) / (e.precision + e.recall)
f2(e::Union{EvaluationResult,WeightedEvaluationResult}) =
    5 * (e.precision * e.recall) / (4 * e.precision + e.recall)
